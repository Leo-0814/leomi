...
export default function AddMovieToListComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ movieListId: ..., movieId: ..., note: ..., position: ..., });
...
## DeleteMyReview
You can execute the `DeleteMyReview` Mutation using the `UseMutationResult` object returned by the following Mutation hook function (which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts)):
```javascript
useDeleteMyReview(options?: useDataConnectMutationOptions<DeleteMyReviewData, FirebaseError, DeleteMyReviewVariables>): UseDataConnectMutationResult<DeleteMyReviewData, DeleteMyReviewVariables>;
```
You can also pass in a `DataConnect` instance to the Mutation hook function.
```javascript
useDeleteMyReview(dc: DataConnect, options?: useDataConnectMutationOptions<DeleteMyReviewData, FirebaseError, DeleteMyReviewVariables>): UseDataConnectMutationResult<DeleteMyReviewData, DeleteMyReviewVariables>;
```
...
```
### Return Type
Recall that calling the `DeleteMyReview` Mutation hook function returns a `UseMutationResult` object. This object holds the state of your Mutation, including whether the Mutation is loading, has completed, or has succeeded/failed, among other things.

To check the status of a Mutation, use the `UseMutationResult.status` field. You can also check for pending / success / error status using the `UseMutationResult.isPending`, `UseMutationResult.isSuccess`, and `UseMutationResult.isError` fields.

To execute the Mutation, call `UseMutationResult.mutate()`. This function executes the Mutation, but does not return the data from the Mutation.

To access the data returned by a Mutation, use the `UseMutationResult.data` field. The data for the `DeleteMyReview` Mutation is of type `DeleteMyReviewData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `DeleteMyReview`'s Mutation hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, DeleteMyReviewVariables } from '@dataconnect/generated';
import { useDeleteMyReview } from '@dataconnect/generated/react'

export default function DeleteMyReviewComponent() {
  // Call the Mutation hook function to get a `UseMutationResult` object which holds the state of your Mutation.
  const mutation = useDeleteMyReview();

  // You can also pass in a `DataConnect` instance to the Mutation hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const mutation = useDeleteMyReview(dataConnect);

  // You can also pass in a `useDataConnectMutationOptions` object to the Mutation hook function.
  const options = {
    onSuccess: () => { console.log('Mutation succeeded!'); }
  };
  const mutation = useDeleteMyReview(options);
  ...
  const mutation = useDeleteMyReview(dataConnect, options);

  // After calling the Mutation hook function, you must call `UseMutationResult.mutate()` to execute the Mutation.
  // The `useDeleteMyReview` Mutation requires an argument of type `DeleteMyReviewVariables`:
  const deleteMyReviewVars: DeleteMyReviewVariables = {
    reviewId: ..., 
  };
  mutation.mutate(deleteMyReviewVars);
  // Variables can be defined inline as well.
  ...
  mutation.mutate(deleteMyReviewVars, options);
